...
class scdisinfact(nn.Module):
    ...
    def reparametrize(self, mu, logvar, clamp = False):
        ...
        for epoch in range(nepochs + 1):
            for data_batch in zip(*self.train_loaders):
                ...
                for diff_factor in range(self.n_diff_factors):
                    ...
                    loss_contr += contr_loss(d_pred.unsqueeze(1), z_ds[diff_factor]["diff_label"].to(self.device))
                ...
                loss = loss_recon + self.lambs["mmd_diff"] * loss_mmd_diff + self.lambs["class"] * (loss_class + 0.1 * loss_contr) + self.lambs["kl"] * loss_kl + self.lambs["gl"] * loss_gl_d + self.lambs["tc"] * loss_tc
                loss.backward()
                self.opt.step()
                self.opt.zero_grad()
                    ...
                    for data_batch in zip(*self.test_loaders):
                        for x in data_batch:
                            ...
                            for diff_factor, (Enc_d, classifier) in enumerate(zip(self.Enc_ds, self.classifiers)):
                                ...
                                # loss_class_test += contr_loss(_z_d, x["diff_labels"][diff_factor].to(self.device))
                        ...
                        for diff_factor in range(self.n_diff_factors):
                            ...
                            # d_pred = F.normalize(self.classifiers[diff_factor](z_ds[diff_factor]["z"]))
                            d_pred = F.normalize(z_ds[diff_factor]["z"])
                            loss_contr_test += contr_loss(d_pred.unsqueeze(1), z_ds[diff_factor]["diff_label"].to(self.device))
                        ...
                        # total loss
                        loss_test = loss_recon_test + self.lambs["mmd_comm"] * loss_mmd_comm_test + self.lambs["mmd_diff"] * loss_mmd_diff_test + self.lambs["class"] * (loss_class_test + 0.1 * loss_contr_test) \
            ...
            for data_batch in zip(*self.train_loaders):
                ...
                for diff_factor in range(self.n_diff_factors):
                    ...
                    if diff_factor == 0:
                        ...
                        # print(torch.unique(z_ds[diff_factor]["diff_label"]))

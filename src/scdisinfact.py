    ...
    def train_joint(self, nepochs = 50, recon_loss = "ZINB"):
        ...
        for epoch in range(nepochs + 1):
            for data_batch in zip(*self.train_loaders):
                ...
                # total loss
                loss = loss_recon + self.lambs[0] * loss_mmd + self.lambs[1] * loss_class + self.lambs[3] * (loss_gl_d+loss_gl_c) + self.lambs[4] * loss_kl + self.lambs[2] * loss_tc
                loss.backward()
                self.opt.step()
                self.opt.zero_grad()                   
            ...
            if epoch % self.interval == 0:
                ...
                with torch.no_grad():
                    ...
                    for data_batch in zip(*self.test_loaders):
                        ...
                        # total loss
                        loss_test = loss_recon + self.lambs[0] * loss_mmd_test + self.lambs[1] * loss_class_test + self.lambs[3] * (loss_gl_d_test+loss_gl_c_test) + self.lambs[4] * loss_kl_test + self.lambs[2] * loss_tc_test_disc

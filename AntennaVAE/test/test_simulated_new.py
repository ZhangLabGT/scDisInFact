...
result_dir = "simulated/ae/"
utils.plot_latent(zs = z_cs_umaps, annos = labels, mode = "joint", save = result_dir + "common_dims_celltypes.png", markerscale = 6)
utils.plot_latent(zs = z_cs_umaps, annos = batches, mode = "joint", save = result_dir + "common_dims_batches.png", markerscale = 6)
utils.plot_latent(zs = z_ts_umaps, annos = labels, mode = "joint", save = result_dir + "time_dims_celltypes.png", markerscale = 6)
utils.plot_latent(zs = z_ts_umaps, annos = timepoints, mode = "joint", save = result_dir + "time_dims_timepoint.png", markerscale = 6)
...
# model1 = new_vae(datasets = datasets, Ks = [12, 4], batch_size = 128, interval = 10, lr = 5e-4, lambs = [1, 1e-3, 10], seed = 0, device = device)
model1 = new_models.new_vae(datasets = datasets, Ks = [12, 4], batch_size = 128, interval = 10, lr = 5e-4, lambs = [1, 1e-2, 10], seed = 0, device = device)
...
with torch.no_grad():
    for x in datasets:
        z_c, _ = model1.Enc_c(x.counts_stand.to(model1.device))
        z_t, _ = model1.Enc_t(x.counts_stand.to(model1.device))
        z_cs.append(z_c.cpu().detach().numpy())
        z_ts.append(z_t.cpu().detach().numpy())
        zs.append(torch.cat((z_c, z_t), dim = 1).cpu().detach().numpy())

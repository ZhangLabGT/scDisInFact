...
meta_cells = pd.concat(meta_cells_array, axis =0, ignore_index = True)
# check same batch different conditions
utils.plot_latent(zs = z_cs_umap, annos = meta_cells["anno"].values.squeeze(), mode = "annos", axis_label = "UMAP", figsize = (10,5), save = result_dir + comment+"common_celltypes.png" if result_dir else None , markerscale = 6, s = 5)
utils.plot_latent(zs = z_cs_umap, batches = meta_cells["batch"].values.squeeze(), mode = "batches", axis_label = "UMAP", figsize = (10,5), save = result_dir + comment+"common_batches.png" if result_dir else None, markerscale = 6, s = 5)
utils.plot_latent(zs = z_cs_umap, annos = meta_cells["condition"].values.squeeze(), mode = "annos", axis_label = "UMAP", figsize = (10,5), save = result_dir + comment+"common_condition.png" if result_dir else None, markerscale = 6, s = 5)
utils.plot_latent(zs = z_cs_umap, annos = meta_cells["anno"].values.squeeze(), batches = meta_cells["batch_cond"].values.squeeze(), mode = "separate", axis_label = "UMAP", figsize = (10,30), save = result_dir + comment+"common_celltypes_separate.png" if result_dir else None , markerscale = 6, s = 5)

for diff_factor in range(model.n_diff_factors):
    utils.plot_latent(zs = z_ds_umap[diff_factor], annos = meta_cells["anno"].values.squeeze(), mode = "annos", axis_label = "UMAP", figsize = (10,5), save = result_dir + comment+f"diff{diff_factor}_celltypes.png" if result_dir else None, markerscale = 6, s = 5)
    utils.plot_latent(zs = z_ds_umap[diff_factor], batches = meta_cells["batch"].values.squeeze(), mode = "batches", axis_label = "UMAP", figsize = (10,5), save = result_dir + comment+f"diff{diff_factor}_batch.png" if result_dir else None, markerscale = 6, s = 5)
    utils.plot_latent(zs = z_ds_umap[diff_factor], annos = meta_cells["condition"].values.squeeze(), mode = "annos", axis_label = "UMAP", figsize = (10,5), save = result_dir + comment+f"diff{diff_factor}_condition.png" if result_dir else None, markerscale = 6, s = 5)
...
scores_scdisinfact["NMI (condition)"] = np.array(nmi_condition_scinsight)
scores_scdisinfact["ARI (condition)"] = np.array(ari_condition_scinsight)
scores_scdisinfact["GC (condition)"] = np.array([gc_condition_scdisinfact] * len(nmi_condition_scinsight))
scores_scdisinfact["Silhouette batch (condition & batches)"] = np.array([silhouette_condition_scdisinfact] * len(nmi_condition_scinsight))
scores_scdisinfact["Silhouette batch (condition & celltype)"] = np.array([silhouette_condition_scdisinfact2] * len(nmi_condition_scinsight))

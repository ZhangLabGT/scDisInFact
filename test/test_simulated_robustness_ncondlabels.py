...
n_batches = 2
simulated_lists = [
  "simulated_robustness_10000_2_3_1_4_20",
  "simulated_robustness_10000_2_4_1_4_20",
  "simulated_robustness_10000_2_5_1_4_20",
  "simulated_robustness_10000_2_3_1_8_20",
  "simulated_robustness_10000_2_4_1_8_20",
  "simulated_robustness_10000_2_5_1_8_20",
]
result_dir = "results_simulated/robustness_ncondlabels/"
# In[]
print("# -------------------------------------------------------------------------------------------")
print("#")
...
for dataset_dir in simulated_lists:
    ...
    for batch_id in range(n_batches):
        ...
        # remove the last condition in batch 1
        if ncondlabels == 3:
            ...
            counts_c3 = pd.read_csv(data_dir + f'GxB{batch_id + 1}_c3.txt', sep = "\t", header = None).values.T
            if batch_id == 0:
                counts_gt[batch_id] = {}
                counts_gt[batch_id]["c1"] = counts_c1[permute_idx[:(3*chunk_size)],:]
                counts_gt[batch_id]["c2"] = counts_c2[permute_idx[:(3*chunk_size)],:]
                counts_gt[batch_id]["c3"] = counts_c3[permute_idx[:(3*chunk_size)],:]
                # training data
                counts_train.append(np.concatenate([
                    counts_c1[permute_idx[:chunk_size],:],
                    counts_c2[permute_idx[chunk_size:(2*chunk_size)],:],
                    counts_c3[permute_idx[(2*chunk_size):(3*chunk_size)],:]], axis = 0))
                # training meta data    
                ...
                meta["condition 1"] = np.array([1] * chunk_size + [2] * chunk_size + [3] * chunk_size)
                ...
                counts_gt[batch_id]["c3"] = counts_c3[permute_idx[:(2*chunk_size)],:]
                # training data
                counts_train.append(np.concatenate([
                    counts_c1[permute_idx[:chunk_size],:],
                    counts_c2[permute_idx[chunk_size:(2*chunk_size)],:]], axis = 0))
                # training meta data    
                meta = pd.DataFrame(columns = ["batch", "condition 1", "annos"])
                meta["batch"] = np.array([batch_id] * chunk_size + [batch_id] * chunk_size)
                meta["condition 1"] = np.array([1] * chunk_size + [2] * chunk_size)
                meta["annos"] = np.concatenate([label_annos[batch_id][permute_idx[:chunk_size]], 
                                                label_annos[batch_id][permute_idx[chunk_size:(2*chunk_size)]]], axis = 0)
                meta_train.append(meta)
            ...
            counts_c2 = pd.read_csv(data_dir + f'GxB{batch_id + 1}_c2.txt', sep = "\t", header = None).values.T
            counts_c3 = pd.read_csv(data_dir + f'GxB{batch_id + 1}_c3.txt', sep = "\t", header = None).values.T
            counts_c4 = pd.read_csv(data_dir + f'GxB{batch_id + 1}_c4.txt', sep = "\t", header = None).values.T
            if batch_id == 0:
                counts_gt[batch_id] = {}
                counts_gt[batch_id]["c1"] = counts_c1[permute_idx[:(4*chunk_size)],:]
                counts_gt[batch_id]["c2"] = counts_c2[permute_idx[:(4*chunk_size)],:]
                counts_gt[batch_id]["c3"] = counts_c3[permute_idx[:(4*chunk_size)],:]
                counts_gt[batch_id]["c4"] = counts_c4[permute_idx[:(4*chunk_size)],:]
                # training data
                counts_train.append(np.concatenate([
                    counts_c1[permute_idx[:chunk_size],:],
                    counts_c2[permute_idx[chunk_size:(2*chunk_size)],:],
                    counts_c3[permute_idx[(2*chunk_size):(3*chunk_size)],:],
                    counts_c4[permute_idx[(3*chunk_size):(4*chunk_size)],:]], axis = 0))
                # training meta data    
                meta = pd.DataFrame(columns = ["batch", "condition 1", "annos"])
                meta["batch"] = np.array([batch_id] * chunk_size + [batch_id] * chunk_size + [batch_id] * chunk_size + [batch_id] * chunk_size)
                meta["condition 1"] = np.array([1] * chunk_size + [2] * chunk_size + [3] * chunk_size + [4] * chunk_size)
                meta["annos"] = np.concatenate([label_annos[batch_id][permute_idx[:chunk_size]], 
                                                label_annos[batch_id][permute_idx[chunk_size:(2*chunk_size)]], 
                                                label_annos[batch_id][permute_idx[(2*chunk_size):(3*chunk_size)]],
                                                label_annos[batch_id][permute_idx[(3*chunk_size):(4*chunk_size)]]], axis = 0)
                meta_train.append(meta)
            elif batch_id == 1:
                counts_gt[batch_id] = {}
                counts_gt[batch_id]["c1"] = counts_c1[permute_idx[:(3*chunk_size)],:]
                counts_gt[batch_id]["c2"] = counts_c2[permute_idx[:(3*chunk_size)],:]
                counts_gt[batch_id]["c3"] = counts_c3[permute_idx[:(3*chunk_size)],:]
                counts_gt[batch_id]["c4"] = counts_c4[permute_idx[:(3*chunk_size)],:]
                # training data
                counts_train.append(np.concatenate([
                    counts_c1[permute_idx[:chunk_size],:],
                    counts_c2[permute_idx[chunk_size:(2*chunk_size)],:],
            ...
            chunk_size = int(ncells/5)
            counts_c1 = pd.read_csv(data_dir + f'GxB{batch_id + 1}_c1.txt', sep = "\t", header = None).values.T
            counts_c2 = pd.read_csv(data_dir + f'GxB{batch_id + 1}_c2.txt', sep = "\t", header = None).values.T
            counts_c3 = pd.read_csv(data_dir + f'GxB{batch_id + 1}_c3.txt', sep = "\t", header = None).values.T
            counts_c4 = pd.read_csv(data_dir + f'GxB{batch_id + 1}_c4.txt', sep = "\t", header = None).values.T
            counts_c5 = pd.read_csv(data_dir + f'GxB{batch_id + 1}_c5.txt', sep = "\t", header = None).values.T
            if batch_id == 0:
                counts_gt[batch_id] = {}
                counts_gt[batch_id]["c1"] = counts_c1[permute_idx[:(5*chunk_size)],:]
                counts_gt[batch_id]["c2"] = counts_c2[permute_idx[:(5*chunk_size)],:]
                counts_gt[batch_id]["c3"] = counts_c3[permute_idx[:(5*chunk_size)],:]
                counts_gt[batch_id]["c4"] = counts_c4[permute_idx[:(5*chunk_size)],:]
                counts_gt[batch_id]["c5"] = counts_c5[permute_idx[:(5*chunk_size)],:]
                # training data
                counts_train.append(np.concatenate([
                    counts_c1[permute_idx[:chunk_size],:],
                    counts_c2[permute_idx[chunk_size:(2*chunk_size)],:],
                    counts_c3[permute_idx[(2*chunk_size):(3*chunk_size)],:], 
                    counts_c4[permute_idx[(3*chunk_size):(4*chunk_size)],:],
                    counts_c5[permute_idx[(4*chunk_size):(5*chunk_size)],:]], axis = 0))
                # training meta data    
                meta = pd.DataFrame(columns = ["batch", "condition 1", "annos"])
                meta["batch"] = np.array([batch_id] * chunk_size + [batch_id] * chunk_size + [batch_id] * chunk_size + [batch_id] * chunk_size + [batch_id] * chunk_size)
                meta["condition 1"] = np.array([1] * chunk_size + [2] * chunk_size + [3] * chunk_size + [4] * chunk_size + [5] * chunk_size)
                meta["annos"] = np.concatenate([label_annos[batch_id][permute_idx[:chunk_size]], 
                                                label_annos[batch_id][permute_idx[chunk_size:(2*chunk_size)]], 
                                                label_annos[batch_id][permute_idx[(2*chunk_size):(3*chunk_size)]],
                                                label_annos[batch_id][permute_idx[(3*chunk_size):(4*chunk_size)]],
                                                label_annos[batch_id][permute_idx[(4*chunk_size):(5*chunk_size)]]], axis = 0)
                meta_train.append(meta)
            elif batch_id == 1:
                counts_gt[batch_id] = {}
                counts_gt[batch_id]["c1"] = counts_c1[permute_idx[:(4*chunk_size)],:]
                ...
                counts_gt[batch_id]["c3"] = counts_c3[permute_idx[:(4*chunk_size)],:]
                counts_gt[batch_id]["c4"] = counts_c4[permute_idx[:(4*chunk_size)],:]
                counts_gt[batch_id]["c5"] = counts_c5[permute_idx[:(4*chunk_size)],:]
                # training data
                counts_train.append(np.concatenate([
                    counts_c1[permute_idx[:chunk_size],:],
                    counts_c2[permute_idx[chunk_size:(2*chunk_size)],:],
    ...
    if ncondlabels == 3:
        idx = (meta_train["condition 1"] == config_input[0]) & (meta_train["batch"] == 1)
        counts_input = counts_train[idx, :]
        meta_input = meta_train.loc[idx, :]
        counts_predict_gt = counts_gt["c3"][idx,:]
        meta_gt = meta_input.copy()
        meta_gt["condition 1"] = 3

    elif ncondlabels == 4:
        idx = (meta_train["condition 1"] == config_input[0]) & (meta_train["batch"] == 1)
        counts_input = counts_train[idx, :]
        meta_input = meta_train.loc[idx, :]
        counts_predict_gt = counts_gt["c4"][idx,:]
        meta_gt = meta_input.copy()
        meta_gt["condition 1"] = 4

    elif ncondlabels == 5:
        idx = (meta_train["condition 1"] == config_input[0]) & (meta_train["batch"] == 1)
        counts_input = counts_train[idx, :]
        meta_input = meta_train.loc[idx, :]
        counts_predict_gt = counts_gt["c5"][idx,:]
        meta_gt = meta_input.copy()
        meta_gt["condition 1"] = 5
    ...
    counts_predict = model.predict_counts(input_counts = counts_input, meta_cells = meta_input, condition_keys = ["condition 1"], 
                                          batch_key = "batch", predict_conds = [ncondlabels], predict_batch = 1)
    ...
    score["Prediction"] = f"{ncondlabels} cond-labels"
    ...
    for cond in range(ncondtypes):
        ...
        auprc_dict = pd.concat([auprc_dict, 
                                pd.DataFrame.from_dict({"Dataset": [dataset_dir], 
                                        ...
                                        "ndiff": [ndiff],
                                        "condition": [cond],
                                        "Prediction": [f"{ncondlabels} cond-labels"]

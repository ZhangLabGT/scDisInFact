...
for dataset_dir in simulated_lists:
    ...
    result_dir = "./results_simulated/partial_latent_batch/" + dataset_dir + "/"
    ...
    for batch_id in range(n_batches):
        ...
        if(batch_id == 0):
            remove_idx = meta_cell.loc[(meta_cell["annos"] == "cell type 16")].index
    ...
    for batch_id in range(n_batches):
        ...
        if(batch_id == 0):
            remove_idx = meta_cell.loc[(meta_cell["annos"] == "cell type 16") | (meta_cell["annos"] == "cell type 10")].index
            meta_cell.drop(remove_idx, inplace = True)
            meta_cell.reset_index(drop=True)
            new_count = np.delete(count, remove_idx, axis = 0)
        else:
...
if True:
    ...
    for dataset_dir in simulated_lists:
        result_dir = "./results_simulated/partial_latent_batch/"
